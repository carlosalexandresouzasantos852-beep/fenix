# =========================
# TICKETS.PY — SISTEMA FARM COMPLETO (FINAL + ESTÁVEL)
# =========================
...
class EntregaModal(discord.ui.Modal, title="📦 Entrega de Farm"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if qtd >= self.meta:
            ...
            motivo = f"Entregou menos da metade da meta — {datetime.now().strftime('%d/%m/%Y')}"
        ...
        if agora.weekday() == 6 and agora.hour == 0 and agora.minute == 0:
            ...
            for cargo_id in cfg["cargos"]:
                ...
                for member in role.members:
                    uid = str(member.id)
                    if uid in entregou or uid in cfg["adv_ativos"]:
                        continue
